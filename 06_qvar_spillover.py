"""
06_qvar_spillover.py — Quantile-VAR Spillover Connectedness Analysis (v6)
==========================================================================
...
def fetch_data(api_key: str) -> pd.DataFrame:
    ...
    try:
        m2 = fred.get_series('WM2NS', observation_start=START, observation_end=END)
    except Exception:
        print("    WM2NS failed, trying M2SL...")
        m2 = fred.get_series('M2SL', observation_start=START, observation_end=END)

    print("  Fetching Gold via yfinance (GC=F)...")
    gold_yf = yf.download("GC=F", start=START, end=END, interval="1mo", progress=False)
    ...
    try:
        oil_daily = fred.get_series('DCOILWTICO', observation_start=START, observation_end=END)
        oil = oil_daily.resample('MS').mean()
    except Exception:
        print("    FRED oil failed, using yfinance CL=F...")
        oil_yf = yf.download("CL=F", start=START, end=END, interval="1mo", progress=False)
    ...
    cs = fred.get_series('CSUSHPINSA', observation_start=START, observation_end=END)

    print("  Fetching S&P 500 (^GSPC)...")
    sp = yf.download("^GSPC", start=START, end=END, interval="1mo", progress=False)
...
def summary_statistics(ret: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in ret.columns:
        ...
        adf_stat, *_ = adfuller(x, maxlag=int(np.floor(12*(n/100)**(1/4))), autolag='AIC')
...
def plot_timeseries(df, save_path):
    ...
    for ax, col, title in zip(axes.flat,
        ...
        ['a) CPI','b) Oil (WTI)','c) Gold','d) M2','e) S&P 500','f) Case-Shiller HPI']):
...
def plot_returns(ret, save_path):
    ...
    for ax, col, title in zip(axes.flat,
        ...
        ax.set_title(title, fontsize=13, fontweight='bold')
        ax.spines['top'].set_visible(False); ax.spines['right'].set_visible(False)
    plt.suptitle("Monthly Log Returns (%)", fontsize=15, fontweight='bold', y=1.01)
    plt.tight_layout(); plt.savefig(save_path, dpi=200, bbox_inches='tight'); plt.close()
    print(f"  ✓ {save_path}")
...
def plot_distributions(ret, save_path):
    ...
    for ax, col, title in zip(axes.flat,
        ...
        ['a) CPI','b) Oil (WTI)','c) Gold','d) M2','e) S&P 500','f) Case-Shiller HPI']):
        x = ret[col].dropna()
        ax.hist(x, bins=40, density=True, color='#cccccc', edgecolor='white')
        kde = gaussian_kde(x); xg = np.linspace(x.min(), x.max(), 200)
        ax.plot(xg, kde(xg), color='black', linewidth=1.5, label='KDE')
        ax.plot(xg, stats.norm.pdf(xg, x.mean(), x.std()), 'r--', linewidth=1, label='Normal')
        ax.set_title(title, fontsize=13, fontweight='bold'); ax.legend(fontsize=8)
        ax.spines['top'].set_visible(False); ax.spines['right'].set_visible(False)
    plt.suptitle("Return Distributions vs Normal", fontsize=15, fontweight='bold', y=1.01)
    plt.tight_layout(); plt.savefig(save_path, dpi=200, bbox_inches='tight'); plt.close()
    print(f"  ✓ {save_path}")
...
def plot_correlation(ret, save_path):
    ...
    ax.set_xticks(range(len(corr))); ax.set_xticklabels(corr.columns, fontsize=11)
    ax.set_yticks(range(len(corr))); ax.set_yticklabels(corr.columns, fontsize=11)
    for i in range(len(corr)):
        for j in range(len(corr)):
            ax.text(j, i, f"{corr.values[i,j]:.2f}", ha='center', va='center', fontsize=10,
                    color='white' if abs(corr.values[i,j]) > 0.5 else 'black')
    plt.colorbar(im, ax=ax, shrink=0.8)
    ax.set_title("Correlation Matrix (Log Returns)", fontsize=14, fontweight='bold')
    plt.tight_layout(); plt.savefig(save_path, dpi=200, bbox_inches='tight'); plt.close()
    print(f"  ✓ {save_path}")

def plot_summary_table(table, save_path):
    fig, ax = plt.subplots(figsize=(14, 4)); ax.axis('off')
    tbl = ax.table(cellText=table.T.values, rowLabels=table.columns,
                   colLabels=table.index, cellLoc='center', loc='center')
    tbl.auto_set_font_size(False); tbl.set_fontsize(10); tbl.scale(1.2, 1.6)
    for (r, c), cell in tbl.get_celld().items():
        if r == 0 or c == -1:
            cell.set_text_props(fontweight='bold'); cell.set_facecolor('#e6e6e6')
    ax.set_title("Summary Statistics (Monthly Log Returns, %)", fontsize=14, fontweight='bold', pad=20)
    plt.tight_layout(); plt.savefig(save_path, dpi=200, bbox_inches='tight'); plt.close()
    print(f"  ✓ {save_path}")
...
def quantile_var_estimate(Y, p, tau):
    ...
    for t in range(p, T):
        ...
        X_rows.append(row); y_rows.append(Y[t])
    X, Ymat = np.array(X_rows), np.array(y_rows)
    coefs, residuals = [], np.zeros((Ymat.shape[0], k))
    for i in range(k):
        res = QuantReg(Ymat[:,i], X).fit(q=tau, max_iter=1000)
        coefs.append(res.params); residuals[:,i] = Ymat[:,i] - X @ res.params
...
def companion_form(phi_list, k):
    p = len(phi_list); Kp = k*p; A = np.zeros((Kp, Kp))
    for j, phi_j in enumerate(phi_list): A[:k, j*k:(j+1)*k] = phi_j
    if p > 1: A[k:, :k*(p-1)] = np.eye(k*(p-1))
    return A
...
def fevd_spillover(phi_list, Sigma, H=10):
    ...
    for h in range(H):
        Theta.append(J @ A_power @ J.T); A_power = A_power @ A
    sigma_diag = np.diag(Sigma); Psi = np.zeros((k, k))
    for i in range(k):
        for j in range(k):
            num, denom = 0.0, 0.0
            ...
            Psi[i,j] = (1.0/sigma_diag[j])*num/denom if denom > 0 else 0
...
def main():
    ...
    df_raw = fetch_data(FRED_API_KEY)
    ret = compute_returns(df_raw)

    # 시각화
    plot_timeseries(df_raw, os.path.join(FIG_DIR, "qvar_timeseries.png"))
    plot_returns(ret, os.path.join(FIG_DIR, "qvar_returns.png"))
    plot_distributions(ret, os.path.join(FIG_DIR, "qvar_distributions.png"))
    plot_correlation(ret, os.path.join(FIG_DIR, "qvar_correlation.png"))

    # 요약 통계
    table = summary_statistics(ret)
    plot_summary_table(table, os.path.join(FIG_DIR, "qvar_summary_table.png"))
    ...
    for tau in [0.05, 0.50, 0.95]:
        ...
        print(f"\n  TCI: {tci:.2f}%")
        tbl.round(2).to_csv(os.path.join(OUT_DIR, f"qvar_spillover_tau_{int(tau*100):02d}.csv"))
