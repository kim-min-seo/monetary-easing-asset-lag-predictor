"""
==================================================
 자산 전이 경로 분석 (통합 독립 실행 버전)
 - 데이터 수집 + 전처리 + 시각화 한 번에 실행
==================================================

[필수 설치]
pip install pandas numpy matplotlib seaborn fredapi yfinance statsmodels scipy

[FRED API 키 발급]
https://fred.stlouisfed.org/docs/api/api_key.html (무료, 1분)
"""
...
FRED_API_KEY = "6c614b89a7238cd94305a17796a45df0"  
...
def fetch_yfinance(ticker: str, col_name: str) -> pd.Series:
    ...
    s.name = col_name
    print(f"  [yfinance] {col_name:<18} {len(s)}개월")
    return s
...
def load_data() -> pd.DataFrame:
    ...
    series = [
        # 자산군
        fetch_fred(fred, "GOLDAMGBD228NLBM", "gold"),
        fetch_yfinance("CL=F",               "wti"),
        fetch_fred(fred, "SP500",             "sp500"),
        fetch_fred(fred, "CSUSHPISA",         "housing"),
        fetch_fred(fred, "CPIAUCSL",          "cpi"),
        # 독립변수
        fetch_fred(fred, "FEDFUNDS",          "fed_rate"),
        fetch_fred(fred, "WALCL",             "fed_balance"),
        fetch_fred(fred, "T10Y2Y",            "yield_curve"),
        fetch_fred(fred, "M2SL",              "m2"),
    ]
    ...
    # 파생변수
    df["fed_rate_chg"] = df["fed_rate"].diff()
...
def calc_cumulative_returns(df: pd.DataFrame,
                             window: int = 36) -> dict:
    results = {}
...
def plot_flow(peak_df: pd.DataFrame):
    ...
    for i, (label, (avg_m, avg_v, col_key)) in enumerate(sorted_assets):
        ...
        ax.text(i, 0.62, f"평균 +{avg_m:.0f}개월",
